...
int NACInit(NSData *certificate, uint64_t *out_val_ctx, NSData **out_session_request) {
    ...
    int ret = rawNACInit(ABSD_PORT, 0x50936603, (vm_offset_t)[certificate bytes], [certificate length], &val_ctx, &session_request, &session_requestCnt);
    if (ret != 0) {
...
int NACKeyEstablishment(uint64_t val_ctx, NSData *session_response) {
    return rawNACKeyEstablishment(ABSD_PORT, 0x50936603, val_ctx, (vm_offset_t)[session_response bytes], [session_response length]);
}
...
int NACSign(uint64_t val_ctx, NSData *data, NSData **out_signature) {
    ...
    int ret = rawNACSign(ABSD_PORT, 0x50936603, val_ctx, (vm_offset_t)[data bytes], [data length], &signature, &signatureCnt);
    if (ret != 0) {
